...
export async function initWebSocketAndConsume(server) {
    const channel = getChannel();
    ...
    const wss = new WebSocketServer({ server }); // Aquí cambia: se adjunta al servidor HTTP
...
function sendNotificationToUser(user_id, data) {
    const socket = clients.get(user_id);
    if (socket && socket.readyState === socket.OPEN) {
        socket.send(JSON.stringify({
            type: "new_notification",
